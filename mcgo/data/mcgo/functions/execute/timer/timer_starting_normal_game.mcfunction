...
execute if score S InGame matches 1 if score NormalMode GameProcess matches 1 if score seconds GameProcess matches 0.. if score StopStartingGame GameProcess matches 0 run title @a title {"text":"游戏即将开始","color":"white","bold":true}
execute if score S InGame matches 1 if score NormalMode GameProcess matches 1 if score seconds GameProcess matches 0.. if score StopStartingGame GameProcess matches 0 run title @a subtitle [{"score":{"name":"seconds","objective":"GameProcess"},"color":"yellow","bold":true}]
execute if score S InGame matches 1 if score NormalMode GameProcess matches 1 if score seconds GameProcess matches 0.. if score StopStartingGame GameProcess matches 0 run playsound block.note_block.hat block @a ~ ~ ~ 1 1 1
