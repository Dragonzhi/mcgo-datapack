...
execute if score CTSurrender Vote < #playersNumSet Vote run execute unless score CTSurrender Vote matches -1 run tellraw @a [{"text":"存在玩家拒绝开始游戏或没有足够的玩家投票，无法开始游戏！","color":"red"}]
execute if score TSurrender Vote < #playersNumSet Vote run execute unless score TSurrender Vote matches -1 run tellraw @a [{"text":"存在玩家拒绝开始游戏或没有足够的玩家投票，无法开始游戏！","color":"red"}]

execute if score CTSurrender Vote = #playersNumSet Vote run scoreboard players set Allowance GameProcess 1
...
execute if score TSurrender Vote = #playersNumSet Vote run scoreboard players set Allowance GameProcess 1
