...
execute if score Voting Vote matches 1 if score @s Vote matches 0 run execute if entity @s[team=T] run scoreboard players set @s Allowance 1
    ...
    execute if score @s Allowance matches 1 run execute if score StartingNormalGame Vote <= #playersNumSet Vote run tellraw @a [{"selector":"@s"},{"text":" 同意开始游戏","color":"green"},{"text":" (","color":"white"},{"score":{"name":"StartingNormalGame","objective":"Vote"},"color":"green"},{"text":" / ","color":"white"},{"score":{"name":"#playersNumSet","objective":"Vote"},"color":"yellow"},{"text":")","color":"white"}]
    execute if score @s Allowance matches 1 run execute if score StartingNormalGame Vote > #playersNumSet Vote run tellraw @a [{"selector":"@s"},{"text":" 同意开始游戏","color":"green"},{"text":" (","color":"white"},{"score":{"name":"StartingNormalGame","objective":"Vote"},"color":"red"},{"text":" / ","color":"white"},{"score":{"name":"#playersNumSet","objective":"Vote"},"color":"yellow"},{"text":")","color":"white"}]
